...
private: 
   ...
   static constexpr size_t width = ceil(log10( (dimension-1) * (dimension-1) ) + 1);
...
public:
   fifteen( )     // Default constructor constructs the puzzle in solved position.
      : open_i{ dimension - 1 },
        open_j{ dimension - 1 }
   {
   ...
   fifteen( std::initializer_list< std::initializer_list< size_t >> init )
   {
      ...
      for(auto i_o = init.begin(); i_o != init.end(); i_o++, i++ )
      {
         size_t j =0;
         for(auto i_i = (*i_o).begin(); i_i != (*i_o).end(); i_i++, j++ )
         {
            table[i][j] = *i_i;
